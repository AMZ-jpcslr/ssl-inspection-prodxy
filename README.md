...
警告ページの「危険性を理解してアクセスする」リンクにはワンタイムトークンが付きます。警告ページを経由していない任意URLを一時許可することはできません。

Dockerや別端末から利用する場合は、警告ページの続行リンクが利用者のブラウザから到達できるように `dashboard.publicBaseUrl`（または環境変数 `DASHBOARD_PUBLIC_BASE_URL`）を設定してください。
